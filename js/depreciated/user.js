$(document).ready(function() {
	...
	jQuery("#register").live("click", function(event) {
		...
		jQuery("#popup").html('<strong>Email</strong>: <input id="registration_email" type="text" name="email" />').dialog( { 
			modal: true,
			title: "Account Registration",
			buttons: [ {
		        text: "OK",
		        click: function() {
			        if($('#registration_email').val() != undefined) {
			      		var email = $('#registration_email').val();
			      		make_dialog_pending(jQuery("#popup"));
			      		//alert($('#registration_email').val());
						send_confirmation(email, slug, type, false);
			        	//$(this).dialog("close");
			        }
				}
			} ] // end buttons
		...
		jQuery("#popup").bind( "dialogbeforeclose", function(event, ui) {
			//
		});
	...
	jQuery("#login").live("click", function(event) {
	
	    $('#login').fadeOut('fast');
	    
		jQuery("#popup").html('<strong>Email</strong>: <input id="login_email" type="text" name="email" /><br/><strong>Password</strong>: <input id="login_password" type="password" name="password" />').dialog( { 
			modal: true,
			title: "Account Registration",
		});
		login_buttons(jQuery("#popup"));
		jQuery("#popup").bind( "dialogbeforeclose", function(event, ui) {
			//
		});
...
function request_login(password) {

	password = md5(password);
	data_to_send = { "method":"request_login", "secret": password};
		...
		if(typeof(data.request_status) != undefined) {
			if(data.result.toLowerCase() == "failure") {
				if(typeof(data.message) != undefined) {
					jQuery("#popup").prepend("<p>" + data.message + "</p>");
				} else {
					jQuery("#popup").prepend('There was an error. Your account is not confirmed.');
				}
				close_button(jQuery("#popup"));
			} else if(data.result.toLowerCase() == "success") {
				if(typeof(data.message) != undefined) {
					jQuery("#popup").html("<p>" + data.message + "</p>");
				} else {
					jQuery("#popup").html('Your account is confirmed.');
				}
				close_button(jQuery("#popup"));
			}
		}
		...
		if(typeof(data.request_status) != undefined) {
			if(data.result.toLowerCase() == "failure") {
				...
				if(typeof(data.reason) != undefined) {
					jQuery("#popup").prepend("<p><small>" + data.reason + "<small></p>");
				}

				if(typeof(data.message) != undefined) {
					jQuery("#popup").prepend("<p>" + data.message + "</p>");
				} else {
					jQuery("#popup").prepend('There was an error.');
				}
			...
			if(data.result.toLowerCase() == "failure") {
				...
				if(typeof(data.reason) != undefined) {
					if(data.reason.toLowerCase() == "account_pending") {
						pending_confirmation_buttons(jQuery( "#popup" ));
					} else {
						ready_to_close_button(jQuery( "#popup" ));
					}
				}
				if(typeof(data.message) != undefined) {
					jQuery("#popup").append("<p>" + data.message + "</p>");
				}

			} else {
				pending_secret_confirmation_buttons( jQuery( "#popup" ) );
				jQuery("#popup").html('Thank you. Buley\'s has sent an email to ' + $('#registration_email').val() + '. Please click the verification link in that email or paste its "secret" into the box below:<br/><br/><strong>Secret</strong>: <input id="confirmation_hash" type="text" name="confirmation_hash" />');
			}
		}
	}, "json");
}
	...
	$.post("/feedback/index.php", data_to_send, function(data) {
		if(typeof(data.result) != undefined) {
			if(data.result.toLowerCase() == "failure") {
				...
				if(typeof(data.message) != undefined) {
					jQuery("<p id='user_message'>" + data.message + "</p>").hide().prependTo("#popup").fadeIn('slow');
				} else {
					jQuery("<p id='user_message'>Incorrect email or password.</p>").hide().prependTo("#popup").fadeIn('slow');
				}
				...
				if(typeof(data.message) != undefined && data.message != null && data.message != '') {
					...
					jQuery('<p id="user_message">Logged in successfully.</p>').hide().prependTo("#popup").fadeIn('slow');
				}
		...
		if(typeof(data.result) != undefined) {
			if(data.result.toLowerCase() == "failure") {
				...
				if(typeof(data.message) != undefined) {
					...
					jQuery("#popup").append("<p>Incorrect email or password.</p>");
				}
...
function make_dialog_pending(dialog) {
	return dialog.html("<center><img src='http://buleys.com/images/ajax-loader.gif' alt='Pending...'/></center>");
}

// Dialog elements

function ready_to_close_button(dialog) {
	var buttons = dialog.dialog( "option", "buttons" );
	dialog.dialog( "option", "buttons", { "Dismiss": function() { $(this).dialog("close"); $("#register").fadeIn('slow'); }  } );
}


function close_button(dialog) {
	var buttons = dialog.dialog( "option", "buttons" );
	dialog.dialog( "option", "buttons", { "Dismiss": function() { $(this).dialog("close"); }  } );
}



function ready_button(dialog) {
	var buttons = dialog.dialog( "option", "buttons" );
	dialog.dialog( "option", "buttons", { "OK": function() { $(this).dialog("close"); }  } );
}

function send_button(dialog) {
	var buttons = dialog.dialog( "option", "buttons" );
	dialog.dialog( "option", "buttons", { "Send": function() { $(this).dialog("close"); }  } );
}

function request_login_buttons(dialog) {
	
	var buttons = dialog.dialog( "option", "buttons" );
	dialog.dialog( "option", "buttons", {
		"Login": function() { 
			if($('#password').val() ) {
				request_login( $('#password_once').val() );
			} else {
...
function request_registration_confirmation_buttons(dialog) {
...
function pending_secret_confirmation_buttons(dialog) {
	
	var buttons = dialog.dialog( "option", "buttons" );
	dialog.dialog( "option", "buttons", {
		"Confirm": function() { 
			if($('#confirmation_hash').val() != '') {
...
function pending_confirmation_buttons(dialog) {
