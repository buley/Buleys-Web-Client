...
InDB.database.load = function( name, description, on_success, on_error, on_abort ) {
	...
	/* End Debug */
	console.log( typeof InDB.db );
	console.log( name === InDB.db.name );
	console.log( InDB.db.name );
	console.log( name );
...
InDB.store.create = function( name, key, autoinc_key, on_success, on_error, on_abort ) {
	...
	setVersionRequest.onsuccess = function( event ) {
		try {
			...
			on_success( event );
			InDB.event.trigger( "InDB_store_created_success", { "name": name, "keyPath": keyPath, "autoinc_key": autoinc_key } );
		} catch( error ) {
			// createdObject store threw an error 
			on_error( error );
			InDB.event.trigger( "InDB_store_created_error", { "name": name, "keyPath": keyPath, "autoinc_key": autoinc_key } );
			//if already created, then the store already exists
			if( IDBDatabaseException.CONSTRAINT_ERR === error.code ) {
				InDB.event.trigger( "InDB_store_already_exists", { "name": name, "keyPath": keyPath, "autoinc_key": autoinc_key } );
			}
		}
	};
